...
public class LayeringCache extends AbstractValueAdaptingCache {
    ...
    public <T> T get(String key, Class<T> resultType) {
        ...
        if (LoggerHelper.isDebugEnabled()) {
            log.info("查询二级缓存,并将数据放到一级缓存。 key={},返回值是:{}", key, GSONUtil.toJson(result));
        }
        return result;
    }
    ...
    private <T> T executeCacheMethod(String key, Class<T> resultType, CacheFunctionWithParamReturn<T,String> valueLoader) {
        while (true) {
            try {
                ...
                if (result != null) {
                    if (LoggerHelper.isDebugEnabled()) {
                        log.info("redis缓存 key= {} 获取到锁后查询查询缓存命中，不需要执行被缓存的方法", key);
                    }
                    return (T) fromStoreValue(result);
                }
                /**获得分布式锁的结果*/
                boolean lockSuccess = redisClient.tryLock(key, layeringCacheConfig.getSecondaryCacheConfig().getWAIT_TIME(),
                        10 * 1000, TimeUnit.MILLISECONDS, () -> {
                            ...
                            if (LoggerHelper.isDebugEnabled()) {
                                log.info("redis缓存 key= {} 从数据库获取数据完毕，唤醒所有等待线程", key);
                            }
                            /**存储到一级缓存中*/
                ...
                if (!lockSuccess) {
                    // 线程等待
                    if (LoggerHelper.isDebugEnabled()) {
                        log.info("redis缓存 key= {} 从数据库获取数据未获取到锁，进入等待状态，等待{}毫秒", key, layeringCacheConfig.getSecondaryCacheConfig().getWAIT_TIME());
                    }
        ...
        ThreadPoolExecutorTask.run(() -> {
            ...
            if (isRefresh(key, preload)) {
                ...
                if (!layeringCacheConfig.getSecondaryCacheConfig().isForceRefresh()) {
                    /**软刷新*/
                    if (LoggerHelper.isDebugEnabled()) {
                        log.info("redis缓存 key={} 软刷新缓存模式", key);
                    }
                    softRefresh(key);
                } else {
                    /**硬刷新*/
                    if (LoggerHelper.isDebugEnabled()) {
                        log.info("redis缓存 key={} 强刷新缓存模式", key);
                    }
